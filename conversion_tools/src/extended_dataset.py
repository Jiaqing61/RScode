...
class ML1MDataset(BaseDataset):
    ...
    def load_item_data(self):
        origin_data = pd.read_csv(self.item_file, delimiter=self.sep, header=None, engine='python', encoding='latin1')
...
class AmazonBeautyDataset(BaseDataset):
    def __init__(self, input_path, output_path):
        ...
        # input files
        self.inter_file = os.path.join(self.input_path, 'All_Beauty.jsonl')
        self.item_file = os.path.join(self.input_path, 'meta_All_Beauty.jsonl')
        ...
        self.inter_fields = {0: 'user_id:token',
                             ...
                             2: 'rating:float',
                             3: 'timestamp:float',
                             4: 'verified_purchase:token',
                             5: 'helpful_vote:token'
                             }

        self.item_fields = {0: 'item_id:token',
                            1: 'title:token',
                            2: 'average_rating:float',
                            3: 'rating_number:token',
                            4: 'main_category:token',
                            5: 'price:float'}
    ...
    def load_inter_data(self):
        interactions = []
        with open(self.inter_file, 'r', encoding='utf-8', errors='ignore') as fin:
            for line in fin:
                ...
                helpful = data.get('helpful_vote', 0)
                interactions.append([user_id, item_id, rating, timestamp, verified, helpful])

        inter_data = pd.DataFrame(interactions)
        return inter_data

    def load_item_data(self):
        items = []
        with open(self.item_file, 'r', encoding='utf-8', errors='ignore') as fin:
            for line in tqdm(fin):
                data = json.loads(line)

                item_id = data.get('parent_asin', '')  # use parent_asin as item_id
                title = data.get('title', '')
                average_rating = data.get('average_rating', 0.0)
                rating_number = data.get('rating_number', 0)
                main_category = data.get('main_category', '')
                price = data.get('price', 0.0)  # Some are null
    ...
    def convert(self, input_data, selected_fields, output_file):
        ...
        for column in selected_fields:
            output_data[selected_fields[column]] = input_data.iloc[:, column]
        output_data.to_csv(output_file, index=False, header=True, sep='\t')
...
class LASTFMDataset(BaseDataset):
    def __init__(self, input_path, output_path):
        ...
        self.dataset_name = 'lastfm-360k'

        # Input files
        self.inter_file = os.path.join(self.input_path, 'usersha1-artmbid-artname-plays.tsv')
        self.user_file = os.path.join(self.input_path, 'usersha1-profile.tsv')
        self.sep = '\t'

        # Output files
        self.output_inter_file, self.output_item_file, self.output_user_file = self.get_output_files()

        # Selected feature mappings
        self.inter_fields = {
    ...
    def load_inter_data(self):
        print(f"Loading interaction data from {self.inter_file} ...")
        interactions = []
        with open(self.inter_file, 'r', encoding='utf-8') as f:
            for line in tqdm(f, desc="Reading interactions"):
                parts = line.strip().split('\t')
                if len(parts) >= 4:
                    user_id, artist_id, _, plays = parts
                    interactions.append((user_id, artist_id, plays))
        inter_data = pd.DataFrame(interactions, columns=['user_id', 'artist_id', 'plays'])
        print(f"Finished loading {len(inter_data)} interaction records.")
        return inter_data

    def load_user_data(self):
        print(f"Loading user profile data from {self.user_file} ...")
        users = []
        with open(self.user_file, 'r', encoding='utf-8') as f:
            for line in tqdm(f, desc="Reading user profiles"):
                parts = line.strip().split('\t')
                if len(parts) >= 4:
                    user_id, gender, age, country = parts[:4]
                    users.append((user_id, gender, age, country))
        user_data = pd.DataFrame(users, columns=['user_id', 'gender', 'age', 'country'])
        print(f"Finished loading {len(user_data)} user profiles.")
        return user_data

    def convert(self, input_data, selected_fields, output_file):
        output_data = pd.DataFrame()
        print(f"Converting fields and writing to {output_file} ...")
        for col_name, output_name in tqdm(selected_fields.items(), desc="Processing columns"):
            output_data[output_name] = input_data[col_name]
    ...
    def convert_inter(self):
        try:
            ...
            print(f"Failed to convert interactions: {e}")
    ...
    def convert_user(self):
        try:
            ...
            print(f"Failed to convert users: {e}")

    def convert_item(self):
        print('No item file to convert for LastFM dataset.')
